...
type RestOf<T extends IDelivery | NewDelivery> = Omit<T, 'request_time' | 'assigned_time' | 'estimated_time' | 'ended_time'> & {
...
export class DeliveryService {
  ...
  protected convertDateFromClient<T extends IDelivery | NewDelivery | PartialUpdateDelivery>(delivery: T): RestOf<T> {
    return {
      ...delivery,
      request_time: delivery.request_time?.toJSON() ?? null,
      assigned_time: delivery.assigned_time?.toJSON() ?? null,
      estimated_time: delivery.estimated_time?.toJSON() ?? null,
      ended_time: delivery.ended_time?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDelivery: RestDelivery): IDelivery {
    return {
      ...restDelivery,
      request_time: restDelivery.request_time ? dayjs(restDelivery.request_time) : undefined,
      assigned_time: restDelivery.assigned_time ? dayjs(restDelivery.assigned_time) : undefined,
      estimated_time: restDelivery.estimated_time ? dayjs(restDelivery.estimated_time) : undefined,
      ended_time: restDelivery.ended_time ? dayjs(restDelivery.ended_time) : undefined,
    };
  }
